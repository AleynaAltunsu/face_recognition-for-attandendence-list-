...
if os.path.exists('Attendance.csv'):
    ...
    print("Attendance.csv dosyası mevcut değil.")
"""
#unkown KİŞİLERİN YOKLAMAYA EKLENMİŞ HALİ   SON VERSİYON
"""
...
#YENİ KİŞİYİ KAYDEDİYOR VS VS SON HALİ
...
while True:
    ...
    for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
        ...
        if name == 'Unknown':
            ...
            cv2.putText(img, "Enter name:", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
            cv2.imshow('Webcam', img)
            cv2.waitKey(1)  # Bekleme süresi ekleme
            new_name = input("Enter the name of the unknown person: ")
            if new_name:
                # Yeni kişiyi tanımlama ve kaydetme
                cv2.imwrite(f"{path}/{new_name}.jpg", img)
                images.append(img)
                classNames.append(new_name)
                encodeListKnown = findEncodings(images)  # Yeni resmi ekleyerek yeniden kodlamaları bulma
                name = new_name
            markAttendance(name)
